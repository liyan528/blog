...
76、什么是JVM？Java虚拟机包括什么？  
是Java虚拟机。Java虚拟机包括：寄存器，堆栈，处理器。  
...
79、Java的数据结构有哪些？  
线性表：ArrayList。链表：LinkedList。栈：stack。队列：Qutue。图：Map。树：Tree。  
...
81、Java中集合框架？  
Java中的集合分为单例集合和双列集合，单例集合顶级接口为collection，双列集合的顶级接口为Map。  
Collection的子接口有list和set这两个。  
List接口的实现类：1、ArrayList:底层实现是数组，查询快，增删慢，线程不安全，效率高。2、LinkedList:底层实现是链表，增删快，查询慢。线程不安全，效率高。  
Set接口的实现类：1、HashSet:底层实现hashMap,数组+链表实现，不允许元素重复。2、TreeSet:底层实现红黑二叉树，实现元素排序。  
Map接口特点：key-value:键值对形式存储数据。  
Map接口的实现类：1、HashMap:底层数组+链表实现，线程不安全，效率高。2、TreeMap:底层是红黑二叉树实现，可实现元素的排序。3、LinkedHashMap:底层hashMap+linkedList实现。  
...
83、重写和重载有什么区别？  
重写发生在不同的类，方法名相同，参数列表相同。重写父方法。  
重载发生在同一个类，方法名相同，参数列表不同。  

84、静态变量和实例变量的区别？  
静态变量是被static修饰符修饰的变量，也称为类变量，它属于类，不属于类的任何一个对象，一个类不管创建多少个对象，静态变量在内存中有且仅有一个拷贝；实例变量必须依赖存于某一个实例，需要先创建对象然后通过对象才能访问到它。静态变量可以实现让每个对象共享内存。 

85、Java中final关键字有哪些用法？  
1、修饰符：表示该类不能被继承；2、修饰方法：表示方法不能被重写；3、修饰变量：表示变量只能一次赋值以后值不能被修改。  

86、Error和Exception有什么区别？  
Error表示系统级的错误和程序不必处理的异常，是恢复不是不可能但很困难的情况下一种严重问题；比如内存溢出，不可能指望程序员能处理这样的情况。  
Exception表示需要捕捉或需要程序进行处理的异常，是一种设计或者实现问题；也就是说，他表示如果程序运行正常，从不会发生的情况。  

87、Collection和Collections的区别？  
Collection是一个接口，他是List、Set等容器的父接口；
Collections是一个工具类，提供了一系列的静态方法来辅助容器操作，这些方法包括对容器的搜索、排序、线程安全化。  

88、线程的sleep方法和yield方法有什么区别？  
1、sleep方法给其他线程运行机会时不考虑线程的优先级，因此会给低优先级的线程运行的机会；yield方法只会给相同优先级或更高优先级的线程的运行机会。  
2、线程执行sleep方法后转入阻塞blocked状态，而执行yield方法后转入就绪ready状态。  
3、sleep方法声明抛出InterruptedException，而yield方法没有声明任何异常。  
4、sleep方法比yield方法具有更好的可移植性。  

89、启动一个线程是调用run还是start方法？  
启动一个线程是调用start方法，使线程所代表的虚拟处理机处于可运行状态，它可以由JVM调度并执行，线程并不会立即运行。run方法是线程启动后进行回调callback的方法。  

90、Java中有几种类型的流？  
字节流和字符流。字节流继承于InputStream、OutputStream,字符流继承reader，writer。在Java.io包中还有许多其他的流，主要是为了提高性能和使用方便。  
Java的I/O需要注意的两点：一是两种对称性；二是两种设计模式。  

91、什么是DAO模式？  
Dao是一个为数据库或其他持久化机制提供了抽象借口的对象，并不在不暴露底层持久化方法实现细节的前提下提供了各种数据访问操作。  

92、简述正则表达式及其用途。  
在编写处理字符串的程序时，经常会有查找符合某些复杂规则的字符串的需要，正则表达式就是用于描述这些规则的工具，换句话说，正则表达式就是记录文本规则的代码。  

93、怎么将byte转换为string？  
可以使用string接收byte参数的构造器来进行转换，需要注意的点事要使用正确的编码，否则会使用平台默认编码，这个编码可能跟原来的编码相同也可能不同。  

94、Java中 ++操作符是线程安全的吗？  
不是线程安全的操作，它涉及到多个指令，如读取变量值，增加然后存储回内存，这个过程可能会出现多个线程交差。  

95、a=a+b 与a+=b的区别？  
+=隐式的将加操作的结果类型强制转换为持有结果的类型，如果这两个整型相加，如byte，short或者int，首先会将它们提升到int类型，然后再执行加法操作，如果加法操作的结果比a的最大值要大，则a+b会出现编译错误，但是a+=b没问题。  

96、3*0.1=0.3将会返回什么？true还是false?  
false，因为有些浮点数不能完全精确的表示出来。  

97、int和integer哪个会占用更多的内存？  
integer对象会占用更多的内存。integer是一个对象，需要存储对象的元数据，但是int是一个原始数据类型的数据，所以占用的空间更少。  

98、为什么Java中的string是不可变得？  
